...
public class FileTestoItems {
    ...
    public static void leggiFiles() throws Exception {

        double i, f, d;

        i = System.currentTimeMillis();

        Scanner fileLemmi = new Scanner(new File("filesWherEXP\\" + configurazioneLemmi + "\\items\\idFraseLemmi.txt"), "UTF-8");

        while(fileLemmi.hasNextLine()){

            String[] riga = fileLemmi.nextLine().split(";");

            String locale = riga[0];
        ...
        Scanner fileIntere = new Scanner(new File("filesWherEXP\\" + configurazioneLemmi + "\\items\\idFrasefrasiIntere.txt"), "UTF-8");

        while(fileIntere.hasNextLine()){

            String[] riga = fileIntere.nextLine().split(";");
